...
def prob_getting_gene(parent, one_gene, two_genes):
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    def compute_prob(person_name):
        if person_name in memo:
            return memo[person_name]
        person = people[person_name]
        trait = person_name in have_trait
        if person["mother"] is None and person["father"] is None:
            if person_name in one_gene:
                prob = PROBS["gene"][1] * PROBS["trait"][1][trait]
            elif person_name in two_genes:
                prob = PROBS["gene"][2] * PROBS["trait"][2][trait]
            else:
                prob = PROBS["gene"][0] * PROBS["trait"][0][trait]
        else:
            mother_prob = prob_getting_gene(person["mother"], one_gene, two_genes)
            father_prob = prob_getting_gene(person["father"], one_gene, two_genes)
            if person_name in one_gene:
                ...
                n_genes = 0
            prob = gene_prob * PROBS["trait"][n_genes][trait]
        memo[person_name] = prob
        return prob
    result = 1
    for person_name in people:
        result *= compute_prob(person_name)
    return result
